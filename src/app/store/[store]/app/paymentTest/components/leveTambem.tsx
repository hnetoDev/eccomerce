...
export default function LeveTambem() {
  ...
  return (
    <div>
      <h1 className="text-xl font-bold">Leve também</h1>
      <div className="border-2 border-dashed border-primary w-max flex p-6 mt-4 rounded-xl bg-primary/5">
        {prods?.map((p, i) => <div key={p.id} className={`p-3 md:p-6 border-primary ${i === (prods.length - 1) ? '' : 'border-r'}  flex-col   justify-between flex`}>
          <div className="flex space-x-2 w-full">
            <Image alt="" width={200} height={200} src={p.image[0]} className="md:w-20 md:h-20 w-12 h-12  rounded-lg" />
